...
def is_truthy(val):
    """Sprawdza, czy wartość z bazy to prawda (obsługuje tekst z excela)"""
...
def init_state(sheet_name):
    """Pobiera dane z chmury do pamięci telefonu (uruchamia się tylko raz)"""
...
def manual_save(sheet_name):
    """Wysyła zaktualizowaną tabelę do Google Sheets"""
    try:
...
def toggle_status_callback(sheet_name, index, col_name, current_val):
    """Magiczny przycisk! Zmienia status, żeby od razu zmienić kolor, a potem zapisuje w tle."""
