...
# Bezpieczne przeładowanie aplikacji (dla starszych i nowszych wersji Streamlit)
def safe_rerun():
    if hasattr(st, "rerun"):
        st.rerun()
    else:
...
# Inicjalizacja błyskawicznej pamięci lokalnej (rozwiązuje problem opóźnień Google Sheets)
def init_state(sheet_name, bool_cols=[]):
    state_key = f"df_{sheet_name}"
    if state_key not in st.session_state:
        try:
            df = load_data(sheet_name)
            # Konwersja tekstów na twarde wartości True/False
            for col in bool_cols:
                if col in df.columns:
                    df[col] = df[col].astype(str).str.strip().str.upper().isin(["TRUE", "1", "YES", "T", "TAK"])
            st.session_state[state_key] = df
        except Exception as e:
            st.error(f"Błąd ładowania danych dla zakładki {sheet_name}: {e}")
...
# Zapisuje stan lokalny z powrotem do Google Sheets
def save_and_sync(sheet_name):
    try:
        df = st.session_state[f"df_{sheet_name}"].copy()
        # Przygotowanie Boole'ów na format czytelny dla Google Sheets
...
if mapa_b64:
    ...
    .stApp {
        ...
        background-size: 100% 100%, 20px 20px; background-attachment: fixed;
        font-family: 'Open Sans', sans-serif !important;
    }
    """
...
# Sprawdzenie czy wyzwolić balony
