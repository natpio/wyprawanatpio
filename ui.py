...
# Nowoczesna biblioteka do stref czasowych (wbudowana w Python 3.9+)
try:
    from zoneinfo import ZoneInfo
except ImportError:
    # Zapasowe wyjście, jeśli serwer byłby starszy
...
def apply_custom_css():
    ...
    .clock-container {{ background: rgba(255, 255, 255, 0.95); border-radius: 12px; padding: 15px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border-top: 4px solid #0B2447; font-family: 'Open Sans', sans-serif; }}
    .clock-time {{ font-family: 'Anton', sans-serif; font-size: 2.5rem; color: #0B2447; line-height: 1.2; margin-bottom: 5px; }}
    </style>
    """
...
def jetlag_clock(city, timezone, weather_str):
    # Stabilny Pythonowy sposób na sprawdzenie godziny (bez użycia JS)
    now = datetime.datetime.now(ZoneInfo(timezone))
    color = '#0B2447' if city=='Poznań' else '#FFC72C' if city=='Chicago' else '#C62828'
    icon = '🇵🇱' if city=='Poznań' else '🇺🇸' if city=='Chicago' else '🌽'
    
    html = f"""
    <div class="clock-container" style="border-top-color: {color};">
        <div style="font-weight: 800; font-size: 1.1rem; color: #0B2447; text-transform: uppercase; margin-bottom: 5px;">{icon} {city}</div>
        <div style="font-size: 1.2rem; color: #C62828; font-weight: 800; margin-bottom: 5px;">{weather_str}</div>
