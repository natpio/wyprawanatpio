...
def safe_rerun():
    """Bezpieczne przeładowanie strony"""
    if hasattr(st, "rerun"):
        st.rerun()
    else:
...
def is_truthy(val):
    """Upewnia się, że statusy z Excela są poprawnie czytane jako Prawda/Fałsz"""
...
def init_state(sheet_name):
    """Ładuje dane z chmury do pamięci telefonu (przy pierwszym odpaleniu)"""
...
def save_and_sync(sheet_name):
    """Pancerna funkcja zapisu do Google Sheets"""
    try:
