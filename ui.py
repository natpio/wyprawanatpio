...
# Obsługa uszkodzonych obrazów
ImageFile.LOAD_TRUNCATED_IMAGES = True

def get_base64_of_bin_file(bin_file):
    """Konwertuje plik binarny na format base64 (potrzebne do CSS i JS)."""
    try:
...
def display_safe_image(filename_base, caption=""):
    """Wyświetla obrazek sprawdzając różne rozszerzenia."""
...
def apply_custom_css():
    """Nakłada style wizualne na aplikację."""
    ...
    @media (max-width: 768px) {{
        .ticket {{ flex-direction: column; }} .ticket-left {{ border-right: none; border-bottom: 3px dashed #cbd5e1; padding-bottom: 25px; }}
        .ticket-header {{ font-size: 1.1rem; flex-direction: column; gap: 8px; text-align: center; }} 
        .ticket-airport {{ font-size: 1.5rem; }} .ticket-airport span {{ font-size: 0.7rem; }}
        .ticket-right {{ min-width: 100%; padding: 15px; }} .countdown-num {{ font-size: 3rem; }}
        .route-card {{ flex-direction: column; align-items: flex-start; }} .route-date {{ border-right: none; border-bottom: 2px dashed #e2e8f0; width: 100%; text-align: left; }}
    }}
    </style>
    """
...
def render_boarding_pass(days_left):
    """Renderuje bilet lotniczy z odliczaniem."""
    html = f"""
            ...
            <div class="ticket-left">
                ...
                <div class="ticket-body">
                    <div class="ticket-row">
                        <div class="ticket-field"><small>Passenger</small><strong>THE CREW (4)</strong></div>
                        <div class="ticket-field"><small>Date</small><strong>30 JUN 2026</strong></div>
                        <div class="ticket-field"><small>Final Destination</small><strong>DES MOINES, IA 🌽</strong></div>
                    </div>
                    <div class="ticket-row" style="margin-top: 15px; align-items: center; justify-content: center; gap: 10px;">
                        <div class="ticket-field"><small>Origin</small><div class="ticket-airport">POZ <span>Poznań</span></div></div>
                        <div style="font-size: 1.2rem;">✈️</div>
                        <div class="ticket-field"><small>Transfer Hub</small><div class="ticket-airport" style="color: #0B2447;">ORD <span>Chicago</span></div></div>
                        <div style="font-size: 1.2rem;">🚗</div>
                        <div class="ticket-field"><small>Destination</small><div class="ticket-airport">DSM <span>Des Moines</span></div></div>
                    </div>
                </div>
            </div>
...
def jetlag_widget(city, flag, weather, timezone, subtitle, border_color):
    """Widget zegara z pogodą i czasem lokalnym."""
    html = f"""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Anton&family=Open+Sans:wght@400;600;800&display=swap');
    .card_jl {{ background: rgba(255, 255, 255, 0.95); border-radius: 12px; padding: 15px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border-top: 4px solid {border_color}; font-family: 'Open Sans', sans-serif; }}
    .city_jl {{ font-weight: 800; font-size: 1.1rem; color: #0B2447; margin-bottom: 5px; text-transform: uppercase; }}
    .weather_jl {{ font-size: 1.4rem; color: #C62828; font-weight: 800; margin-bottom: 5px; }}
    .clock_jl {{ font-family: 'Anton', sans-serif; font-size: 2.5rem; color: #0B2447; line-height: 1.2; margin-bottom: 5px; }}
    .sub_jl {{ font-size: 0.75rem; color: #64748b; font-weight: 800; text-transform: uppercase; }}
    </style>
    <div class="card_jl">
        <div class="city_jl">{flag} {city}</div>
        <div class="weather_jl">{weather}</div>
        <div class="clock_jl" id="clock_{city.replace(' ','')}">--:--:--</div>
        <div class="sub_jl">{subtitle}</div>
    </div>
    <script>
...
def render_customs_card(d):
    """Renderuje kartę podglądu deklaracji celnej."""
    html = f"""
    <div class="customs-card">
        <h3 class="customs-title">CUSTOMS DECLARATION</h3>
        <div class="customs-sub">CBP Form 6059B (Cheat Sheet)</div>
        <div class="customs-line"><span class="c-num">1.</span> Name: <span class="c-ans">{d['c_last']} {d['c_first']}</span></div>
        <div class="customs-line"><span class="c-num">2.</span> DOB: <span class="c-ans">{d['c_dob']}</span></div>
        <div class="customs-line"><span class="c-num">3.</span> Family Members: <span class="c-ans">{d['c_mem']}</span></div>
        <div class="customs-line"><span class="c-num">4.</span> Address: <span class="c-ans">{d['c_street']} {d['c_city']} {d['c_state']}</span></div>
        <div class="customs-line"><span class="c-num">5-6.</span> Passport: <span class="c-ans">{d['c_pass_country']} {d['c_pass_no']}</span></div>
        <div class="customs-line"><span class="c-num">7.</span> Residence: <span class="c-ans">{d['c_residence']}</span></div>
        <div class="customs-line"><span class="c-num">8.</span> Visited: <span class="c-ans">{d['c_visited']}</span></div>
        <div class="customs-line"><span class="c-num">9.</span> Flight: <span class="c-ans">{d['c_fly']}</span></div>
        <div style="margin-top: 10px; font-weight: bold; color: #0284c7; font-size: 0.8rem;">QUESTIONS 11 - 14: ALL NO (X)</div>
        <div style="margin-top: 10px;"><span class="c-num">15.</span> Value of Gifts: <span class="c-ans">${d['c_15']}</span></div>
    </div>
    """
    st.markdown(html.replace('\n', ''), unsafe_allow_html=True)

def set_mobile_icon(image_path="ikona.png"):
    """Zmienia ikonę aplikacji widoczną po dodaniu do ekranu głównego na telefonie."""
    b64_icon = get_base64_of_bin_file(image_path)
    if b64_icon:
        js_code = f"""
        <script>
        const doc = window.parent.document;
        const head = doc.getElementsByTagName('head')[0];
        const iconUrl = "data:image/png;base64,{b64_icon}";
        ...
        // Android / Chrome
        let stdIcon = doc.querySelector('link[rel="icon"]');
        if (!stdIcon) {{
            stdIcon = doc.createElement('link');
            stdIcon.rel = 'icon';
            head.appendChild(stdIcon);
        }}
        stdIcon.href = iconUrl;
        </script>
        """
